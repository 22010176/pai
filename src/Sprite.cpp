...
Sprite::Sprite(Renderer& renderer, SDL_Surface* surface, double angle) : renderer(renderer), angle(angle), width(surface->w), height(surface->h) {
...
Sprite::Sprite(Renderer& renderer, SDL_Texture* texture, double angle) : renderer(renderer), angle(angle) {
    SDL_QueryTexture(texture, nullptr, nullptr, &this->width, &this->height);
    this->texture = SDL_CreateTexture(renderer.GetRenderer(), SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, width, height);
    SDL_SetTextureBlendMode(this->texture, SDL_BLENDMODE_BLEND);
    AddTexture(texture, { 0,0,width,height });
}
Sprite::Sprite(Renderer& renderer, SDL_Rect rect, SDL_Color color) : renderer(renderer), pos(rect), width(rect.w), height(rect.h) {
    this->texture = SDL_CreateTexture(renderer.GetRenderer(), SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, width, height);
    SDL_SetTextureBlendMode(this->texture, SDL_BLENDMODE_BLEND);
    AddTexture(rect, color);
}
Sprite::Sprite(Renderer& renderer, int width, int height) : renderer(renderer), width(width), height(height) {
    this->texture = SDL_CreateTexture(renderer.GetRenderer(), SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, width, height);
    SDL_SetTextureBlendMode(this->texture, SDL_BLENDMODE_BLEND);
}
Sprite::~Sprite() { if (this->texture != nullptr) SDL_DestroyTexture(this->texture); }
